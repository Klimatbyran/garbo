...
This is the main repository for the AI bot we call Garbo. Garbo is a bot that can run via our UI [Validation frontend](https://github.com/Klimatbyran/validate-frontend). Garbo is powered by LLM:s to fetch and extract GHG self-reported data from companies. It automates the process of data extraction, evaluation, and formatting, providing a streamlined workflow for handling environmental data.

Garbo is invoked in our UI and has a pipeline of tasks and jobs that will be started in order for her to both extract, evaluate and format the data autonomously.
...
Start the validation frontend and pipeline-api to use our custom UI.
...
To run the Garbo pipeline for processing sustainability reports, you'll need to use the validation tool frontend.

#### Running a Report Through the Pipeline

Run the application using `npm run dev` or `npm run dev-board` + `npm run dev-workers`.
- Start the validation frontend and follow the instructions to run reports via the frontend. 
