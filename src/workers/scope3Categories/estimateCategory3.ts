...
export class EstimateCategory3Job extends DiscordJob {
...
const estimateCategory3 = new DiscordWorker<EstimateCategory3Job>(
  'estimateCategory3',
  async (job) => {
    ...
    } = await import('../../prompts/scope3Categories/category3')

    // Get relevant context about energy consumption and related activities
    ...
    const response = await askStream(
      [
        {
          role: 'system',
          content: 'You are an expert in Scope 3 Category 3 emissions calculation and energy-related activities analysis.'
        },
        {
          role: 'user',
          content: `Analyze Category 3 (Fuel and Energy-Related Activities) emissions for ${companyName}:\n${JSON.stringify(contextData, null, 2)}\n\n${prompt}`
        }
      ],
      {
...
📊 Scope 3 Category 3 Analysis for ${companyName}:
...
export default estimateCategory3
