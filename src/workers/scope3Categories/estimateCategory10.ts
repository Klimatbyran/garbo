...
export class EstimateCategory10Job extends DiscordJob {
...
const estimateCategory10 = new DiscordWorker<EstimateCategory10Job>(
  'estimateCategory10',
  async (job) => {
    ...
    } = await import('../../prompts/scope3Categories/category10')

    // Get relevant context about product processing
    ...
    const response = await askStream(
      [
        {
          role: 'system',
          content: 'You are an expert in Scope 3 Category 10 emissions calculation and product processing analysis.'
        },
        {
          role: 'user',
          content: `Analyze Category 10 (Processing of Sold Products) emissions for ${companyName}:\n${JSON.stringify(contextData, null, 2)}\n\n${prompt}`
        }
      ],
      {
        response_format: zodResponseFormat(schema, 'scope3-category10')
      }
    )
...
📊 Scope 3 Category 10 Analysis for ${companyName}:
...
export default estimateCategory10
