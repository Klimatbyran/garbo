...
export class EstimateCategory9Job extends DiscordJob {
...
const estimateCategory9 = new DiscordWorker<EstimateCategory9Job>(
  'estimateCategory9',
  async (job) => {
    ...
    } = await import('../../prompts/scope3Categories/category9')

    // Get relevant context about downstream transportation
    ...
    const response = await askStream(
      [
        {
          role: 'system',
          content: 'You are an expert in Scope 3 Category 9 emissions calculation and downstream transportation analysis.'
        },
        {
          role: 'user',
          content: `Analyze Category 9 (Downstream Transportation and Distribution) emissions for ${companyName}:\n${JSON.stringify(contextData, null, 2)}\n\n${prompt}`
        }
      ],
      {
        response_format: zodResponseFormat(schema, 'scope3-category9')
      }
    )
...
📊 Scope 3 Category 9 Analysis for ${companyName}:
...
export default estimateCategory9
