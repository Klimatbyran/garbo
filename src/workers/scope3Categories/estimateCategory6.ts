...
export class EstimateCategory6Job extends DiscordJob {
...
const estimateCategory6 = new DiscordWorker<EstimateCategory6Job>(
  'estimateCategory6',
  async (job) => {
    ...
    } = await import('../../prompts/scope3Categories/category6')

    // Get relevant context about business travel
    ...
    const response = await askStream(
      [
        {
          role: 'system',
          content: 'You are an expert in Scope 3 Category 6 emissions calculation and business travel analysis.'
        },
        {
          role: 'user',
          content: `Analyze Category 6 (Business Travel) emissions for ${companyName}:\n${JSON.stringify(contextData, null, 2)}\n\n${prompt}`
        }
      ],
      {
        response_format: zodResponseFormat(schema, 'scope3-category6')
      }
    )
...
📊 Scope 3 Category 6 Analysis for ${companyName}:
...
export default estimateCategory6
