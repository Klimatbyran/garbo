...
const summarizeCategories = new DiscordWorker<SummarizeCategoriesJob>(
  ...
  async (job) => {
    ...
    const childrenValues = (await job.getChildrenEntries()) as Record<
      string,
      Scope3CategoryAnalysis
    >

    // Convert category analyses into scope3 data structure
    const categories = Object.entries(childrenValues).map(
      ([queueName, analysis]) => ({
...
${categories
  .map((c) => `Category ${c.category}: ${c.total} ${c.unit}`)
  .join('\n')}
`)
