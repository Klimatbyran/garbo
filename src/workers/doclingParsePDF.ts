...
type ProcessUrlAsyncResponse =
  paths['/v1alpha/convert/source/async']['post']['responses']['200']['content']['application/json']
type TaskStatusResponse =
  paths['/v1alpha/status/poll/{task_id}']['get']['responses']['200']['content']['application/json']
type TaskResultResponse =
  paths['/v1alpha/result/{task_id}']['get']['responses']['200']['content']['application/json']
  ...
  async (job: DoclingParsePDFJob) => {
    ...
    try {
      ...
      try {
        ...
        const startResponse = await fetch(docling.baseUrl, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            authorization: `Bearer ${docling.BERGET_AI_TOKEN}`,
          },
  ...
  while (markdown === undefined) {
    ...
    if (statusResponse.status === 200) {
      ...
      const retryAfter = parseInt(
        statusResponse.headers.get('retry-after') || '2',
      )
