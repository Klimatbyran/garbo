...
const worker = new DiscordWorker('userFeedback', async (job: JobData) => {
  const { feedback, url, json: previousJson } = job.data

  job.sendMessage(`🎯 Feedback: ${feedback} ${job.attemptsStarted || ''}`)
  ...
  job.sendMessage(json ? response.replace(json, '...json...') : response)

  if (Object.keys(parsedJson)) {
    job.log('Parsed JSON: ' + JSON.stringify(parsedJson, null, 2))

    if (parsedJson.agentResponse)
      await job.sendMessage(parsedJson.agentResponse)
    if (parsedJson.reviewComment)
      await job.sendMessage(parsedJson.reviewComment)
