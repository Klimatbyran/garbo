...
class AuthService {
  async authorizeUser(code: string, redirectUri?: string) {
    ...
    const accessTokenRes = await axios.post<{ access_token: string }>(
    ...
    const userinfoRes = await axios.get<GithubUserinfo>(
  ...
  verifyToken(token: string) {
    const userPayload = jwt.verify(token, apiConfig.jwtSecret) as User & {
      exp: number
    }
    ...
    const renewalThreshold =
      AuthService.TOKEN_EXPIRY_BUFFER_MINUTES * AuthService.SECONDS_IN_A_MINUTE

    const shouldRenew =
      currentTimeInSeconds > userPayload.exp - renewalThreshold
