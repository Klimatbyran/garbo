...
const getCompanyEmissionsDataFromDatabase = async (wikidataId: string, years: string[]): Promise<CompanyWithEmissions | null> => {
...
const companyIsValid = (company: CompanyWithEmissions): { valid: boolean, message: string } => {
  ...
  const mostRecentPeriod = company.reportingPeriods[company.reportingPeriods.length - 1]
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        ...
        const company = await getCompanyEmissionsDataFromDatabase(wikidataId, years)
        ...
        const reportingPeriods = company.reportingPeriods as ReportingPeriodData[]
        const mostRecentPeriod = company.reportingPeriods[company.reportingPeriods.length - 1]
