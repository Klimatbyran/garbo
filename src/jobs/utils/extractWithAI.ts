...
export const extractDataFromMarkdown = async (markdown: string, type: string, prompt: string, schema: z.ZodSchema) => {
    try {
        ...
        return JSON.stringify({ error: { name, message, stack, stage: 'askStreamWithContext', type } })
    }
}
...
export const extractDataFromUrl = async (url: string, type: string, prompt: string, schema: z.ZodSchema, queryTexts: string[]) => {
    ...
    try {
        ...
        const name = error?.name || 'Error'
        const stack = error?.stack || undefined
        return JSON.stringify({ error: { name, message, stack, stage: 'getRelevantMarkdown', type, url } })
    }
...
/*     job.log(`Reflecting on: ${prompt}
    ...
    try {
        ...
        const name = error?.name || 'Error'
        const stack = error?.stack || undefined
        return JSON.stringify({ error: { name, message, stack, stage: 'extractDataFromMarkdown', type, url } })
    }
  }
