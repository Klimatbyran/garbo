...
const envSchema = z.object({
  ...
  USE_BACKUP_API: z.enum(['true', 'false']).transform((v) => v === 'true').optional().default('false'),
  BACKUP_API_URL: z.string().optional(),
  BACKUP_API_TOKEN: z.string().optional(),
  BACKUP_API_AUTH_HEADER: z.enum(['authorization', 'x-api-key']).optional().default('authorization'),
})
...
if (!parsedEnv.success) {
  ...
  if (parsedEnv.error.errors.some((err) => err.path[0] === 'DOCLING_URL')) {
    ...
    console.error('Examples: http://localhost:5001/v1 (local), https://api.example.com (remote)')
    console.error(
...
const { DOCLING_USE_LOCAL, BERGET_AI_TOKEN, DOCLING_URL, USE_BACKUP_API, BACKUP_API_URL, BACKUP_API_TOKEN, BACKUP_API_AUTH_HEADER } = parsedEnv.data
...
if (USE_BACKUP_API && (!BACKUP_API_URL || !BACKUP_API_TOKEN)) {
  throw new Error('USE_BACKUP_API is true but BACKUP_API_URL or BACKUP_API_TOKEN is missing')
}
