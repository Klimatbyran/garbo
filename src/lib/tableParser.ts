...
const findMostDetailedHeaderRow = (headerRows: any[]): any => {
  ...
  headerRows.forEach((row) => {
    ...
    row.cells?.forEach((cell: any) => {
      ...
      if (/\b(1[89]\d{2}|20[0-9]\d|21\d{2})\b/.test(text) || // 1800-2199
          /\b'\d{2}\b/.test(text) || // '99, '03
          /\b(19|20)\d{2}s?\b/.test(text)) { // 1990s
        yearScore += 50
      }
    })
...
const createGroupingHeaderRows = (headerRows: any[], mainHeaderRow: any): string[] => {
...
const processDataRows = (dataRows: any[]): string[] => {
  return dataRows.map((row) => {
    if (row.type === 'full_row') {
      ...
      const cells = row.cells?.map((cell: any) => getCellValueString(cell)) || []
      return `| ${cells.join(' | ')} |`
    }
  })
}
...
export const parseTable = (block: NLMIngestorTable) => {
  ...
  if (headerRows.length > 0) {
    ...
    const groupingRows = createGroupingHeaderRows(headerRows, mostDetailedHeaderRow)
