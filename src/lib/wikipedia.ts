...
export function generateWikipediaArticleText(emissions: Emissions, title: string, language: string): string {
    switch (language) {
        case "en":
            let enText = `During ${emissions.reportingPeriod.year} the total amount of greenhouse gas emissions generated by ${title} added up to`
            ...
            return enText
        case "sv":
            let svText = `Under ${emissions.reportingPeriod.year} uppgick de totala utsläppen av växthusgaser för ${title} till`
            if (emissions.statedTotalEmissions.total && emissions.statedTotalEmissions.unit) {
