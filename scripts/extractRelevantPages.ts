...
function findRelevantPages(pdfBuffer: Buffer): {
  pages: number[]
...
async function main() {
  ...
  console.log(
    `Loaded PDF (${(pdfBuffer.byteLength / 1024 / 1024).toFixed(1)} MB)\n`
  )
  ...
  const jsonOutputPath = path.resolve(
    'public/atlas-copco-test-report-relevant-pages.json'
  )
  fs.writeFileSync(
    jsonOutputPath,
    JSON.stringify(
  ...
  const pdfOutputPath = path.resolve(
    'public/atlas-copco-test-report-relevant-pages.pdf'
  )
