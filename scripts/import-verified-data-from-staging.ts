...
function filterObjectsWithVerifiedBy(
  ...
  return reportingPeriods.map(period => {
    ...
    if(emissions) {
      ...
      if (emissions?.scope3?.statedTotalEmissions && emissions.scope3?.statedTotalEmissions.metadata.verifiedBy) {
        if(!reportingPeriod.emissions) {
          reportingPeriod.emissions = {};
        }
  ...
  for (const company of companies) {
    ...
    if (!response.ok) {
      ...
      if(response.status === 400) {
        console.log(JSON.stringify({ reportingPeriods: filteredReportingPeriods }));
      } else {
