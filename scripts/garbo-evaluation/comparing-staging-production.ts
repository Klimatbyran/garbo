...
function compareCompanyLists(
  ...
  for (const productionCompany of productionCompanies) {
    const stagingCompany = stagingCompanies.find(
      (companyCandidate) =>
        companyCandidate.wikidataId === productionCompany.wikidataId
    )
...
function addDiffForSingleReportingYear(params: {
  ...
  const productionReportingPeriod = productionCompany.reportingPeriods.find(
    (period) => {
...
function compareReportingPeriods(
  ...
  diffReport.diffs.emissions.statedTotalEmissions = compareNumbers(
    ...
    productionReportingPeriod.emissions?.statedTotalEmissions?.metadata
      .verifiedBy != null
  )
  ...
  for (let i = 1; i <= NUMBER_OF_CATEGORIES; i++) {
    const productionCategory =
      productionReportingPeriod.emissions?.scope3?.categories.find(
        (categoryI) => categoryI.category === i
      ) ?? undefined
    const stagingCategory =
      stagingReportingPeriod.emissions?.scope3?.categories.find(
        (categoryI) => categoryI.category === i
      ) ?? undefined
    const diff = compareNumbers(
      productionCategory?.total,
      stagingCategory?.total,
      productionCategory?.metadata.verifiedBy != null
    )
    diffs.push(diff)
...
function compareNumbers(
  productionNumber: number | undefined | null,
  stagingNumber: number | undefined | null,
  ...
  if (stagingNumber && productionNumber) {
    ...
    diff.differencePerct =
      Math.ceil((diff.difference / productionNumber) * 100) / 100
  }
...
async function main() {
  try {
    ...
    const companies = compareCompanyLists(
      productionData,
      stagingData,
      reportingYear
    )
