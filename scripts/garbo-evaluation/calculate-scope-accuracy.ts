...
function hasAnyValue(diff: Diff | undefined): boolean {
  ...
  const hasProd =
    diff.productionValue !== undefined && diff.productionValue !== null
  const hasStaging =
    diff.stagingValue !== undefined && diff.stagingValue !== null
...
function is2024Report(report: DiffReport): boolean {
  const startDate =
    typeof report.reportingPeriod.startDate === 'string'
...
function extractScope3Diffs(companies: Company[]): Diff[] {
  ...
  for (const company of companies) {
    for (const report of company.diffReports) {
      ...
      const scope3StatedTotal = (report.diffs.emissions as any)
...
function calculateAccuracy(diffs: Diff[], label?: string): AccuracyMetrics {
  ...
  if (label === 'scope2') {
    ...
    console.log(
      `🔢 Breakdown: ${both} both values, ${prodOnly} prod only, ${stagingOnly} staging only, ${neither} neither`
    )
  }
 ...
 */
function formatAccuracyMetrics(
...
function displayResults(results: ScopeAccuracyResults): void {
  ...
  console.log(
    formatAccuracyMetrics(
      results.scope1And2,
      'Scope 1 & 2 Combined (includes individual + combined field)'
    )
  )
  ...
  console.log(
    formatAccuracyMetrics(
      results.overall,
      'Overall (Scope 1&2 + Scope 3 + Totals)'
    )
  )
}
