...
export function outputVerificationCounts(productionCompanies: CompanyList, reportingYear?: string) {
  const counts: Record<
    ...
    for (const period of company.reportingPeriods) {
      ...
      if (reportingYear) {
        const periodStartDate =
          typeof period.startDate === 'string' ? period.startDate : period.startDate.toString();
